...
11. [Оркестратор и специалисты](#11-оркестратор-и-специалисты)
12. [MCP-инструменты](#12-mcp-инструменты)
13. [Правила поведения агента](#13-правила-поведения-агента)
...
| snowflake-arctic-embed2 | ~670 MB | Мультиязычные эмбеддинги (1024d) |
| **Итого** | **~22 GB** | |
...
## 11. Оркестратор и специалисты

### 11.1. Как работает оркестратор

Каждый запрос автоматически маршрутизируется к подходящему специалисту:

```
"Какой баланс на WB?" → Orchestrator → WB Finance → wb_get_balance → ответ
"Покажи карточки товаров" → Orchestrator → WB Content → wb_get_cards_list → ответ
```

### 11.2. Специалисты

| Специалист | Ключевые слова | Инструменты |
|-----------|---------------|-------------|
| WB Analytics | продажи, воронка, конверсия, поиск | 6 |
| WB Pricing | цены, скидки, карантин | 7 |
| WB Stock & Logistics | склад, остатки, заказы, FBS, поставки | 28 |
| WB Content | карточки, товары, категории, теги | 12 |
| WB Finance | баланс, финансы, отчёт, реализация | 6 |

### 11.3. Тестирование маршрутизации

В Admin Panel → Agent → введите запрос в поле "Test Routing" → увидите какой специалист выбран и scores для всех.

```bash
# Через API
curl -X POST http://localhost:8100/admin/specialists/test \
  -H "Content-Type: application/json" \
  -d '{"query": "покажи остатки на складе"}'
# → {"selected": "stock_logistics", "specialist_name": "WB Stock & Logistics", ...}
```

### 11.4. Добавление нового специалиста

Отредактируйте `agent/orchestrator.py` → словарь `SPECIALISTS`:
...
### 12.1. Подключение MCP-сервера

Admin Panel → MCP Tools → введите имя, URL (с `/sse` для SSE-транспорта), опциональный API-ключ → **Add & Discover**.

```bash
# Или через API
curl -X POST http://localhost:8100/admin/mcp/servers \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer ваш_ключ" \
  -d '{"name": "wildberries", "url": "http://localhost:8000/sse"}'
```

### 12.2. Управление инструментами

- **Включение/выключение** отдельных инструментов — toggle в карточке сервера
- **Refresh** — повторное обнаружение инструментов
- **Delete** — удаление сервера и всех его инструментов

### 12.3. Как агент использует MCP

1. Оркестратор выбирает специалиста по запросу
2. Специалист получает отфильтрованные MCP-инструменты (max 15)
3. LLM решает какой инструмент вызвать
4. Агент выполняет вызов через MCP SSE (initialize → tools/call)
5. Результат возвращается LLM для формирования ответа
6. Цикл повторяется до 5 раз для сложных запросов

### 12.4. Важные замечания

- MCP-серверы на хосте доступны из Docker через `host.docker.internal`
- URL должен содержать `/sse` для SSE-транспорта
- На Windows: переменные окружения MCP-сервера (например `WB_API_KEY`) должны быть установлены через `set` в **той же** консоли где запущен сервер

---

## 13. Правила поведения агента

### 13.1. Настройки агента

Admin Panel → Agent → Agent Settings:

- **System Prompt** — основной промпт агента
- **Personality** — стиль общения (например "professional, concise")
- **Language** — языковые инструкции (например "always respond in Russian")

### 13.2. Behavior Rules

Список правил, которые агент ОБЯЗАН соблюдать:

```bash
# Добавление правила через API
curl -X POST http://localhost:8100/admin/agent/rules \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer ваш_ключ" \
  -d '{"rule_text": "Всегда цитируй источники", "category": "formatting", "priority": 10}'
```

Категории: `general`, `safety`, `formatting`, `domain`.

Правила с высоким priority применяются первыми.

### 13.3. Превью промпта

Admin Panel → Agent → **Refresh Preview** — показывает итоговый промпт, который получает LLM (settings + rules + specialist prompt).
...
Admin Panel → Activity — визуальная цепочка для каждого запроса:

```
🧭 ROUTE     → WB Finance (6 tools)
🔧 TOOL_CALL → wb_get_balance — 10238ms
   Args: {}
   Result: {"currency":"RUB","current":70424.19}
💬 ANSWER    → "Ваш баланс 70 424.19 руб" — 118s total
```

Записи группируются по сессии, раскрываются по клику.

### 14.2. API

```bash
# Получить последние 50 записей
curl http://localhost:8100/admin/activity?limit=50

# Фильтр по сессии
curl http://localhost:8100/admin/activity?session_id=xxx

# Очистить лог
curl -X DELETE http://localhost:8100/admin/activity \
  -H "Authorization: Bearer ваш_ключ"
```
...
| Шаг | Данные |
|-----|--------|
| route | Выбранный специалист, запрос, количество tools |
| tool_call | Имя инструмента, аргументы, результат, duration_ms |
| answer | Финальный ответ, общее время |
